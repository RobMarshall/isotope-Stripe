...
class Stripe extends Postsale implements IsotopePayment {
    ...
    public function processPostsale(IsotopeProductCollection $objOrder) {
        ...
        if (\Input::post('requestFingerprint') == $this->calcHashPost()) {
            \System::log('The given hash does not match for Order ID "' . \Input::post('order_id') . '" (Stripe)', __METHOD__, TL_ERROR);
        ...
        // log
        \System::log('Update of payment status of Order ID "' . \Input::post('order_id') . '" (Stripe): "' . $strState . '"', __METHOD__, TL_GENERAL);
        ...
        if (!$objOrder->checkout()) {
            \System::log('Stripe checkout for Order ID "' . \Input::post('order_id') . '" failed', __METHOD__, TL_ERROR);
    ...
    public function checkoutForm(IsotopeProductCollection $objOrder, \Module $objModule) {
        ...
        $objTemplate = new \Isotope\Template('iso_payment_stripe');
        $objTemplate->setData($this->arrData);
        $objTemplate->action = 'https://checkout.stripe.com/page/init.php';
